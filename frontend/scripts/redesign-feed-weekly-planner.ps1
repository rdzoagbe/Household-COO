# Household COO - Redesign Feed Weekly Planner
# Promotes Next Up / This Week into a compact actionable weekly agenda.
# Removes duplicate capture actions from the Feed page.
...
# Remove capture-specific UI blocks from Feed.
...
# Remove capture-specific modal blocks.
...
$content = $content.Replace("return Boolean(time && time >= now && time <= tomorrow);", "return Boolean(time && time >= now && time <= weekEnd);")
...
'@
  $content = $content.Replace("`n  const childMembers = useMemo(() => members.filter((m) => m.role?.toLowerCase() === 'child'), [members]);", $agendaBlock + "`n  const childMembers = useMemo(() => members.filter((m) => m.role?.toLowerCase() === 'child'), [members]);")
}
