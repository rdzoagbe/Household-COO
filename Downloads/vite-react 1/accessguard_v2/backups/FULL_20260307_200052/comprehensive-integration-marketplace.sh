#!/bin/bash
# ============================================================================
# COMPREHENSIVE INTEGRATION MARKETPLACE - OPTION A
# ============================================================================
# Adds usage stats, badges, progress bars, cost insights, and more

set -e

echo "🎨 Building Comprehensive Integration Marketplace"
echo "================================================="
echo ""
...
echo "🔧 Creating enhanced IntegrationConnectors with full stats..."

# Create the comprehensive component directly in App.jsx location
cat > /tmp/integration_full.js << 'INTEGRATION_EOF'
...
echo "  ✅ Comprehensive component created"
echo ""

echo "🔧 Finding and replacing IntegrationConnectors in App.jsx..."

# Find the line where IntegrationConnectors starts
CONNECTOR_START=$(grep -n "^function IntegrationConnectors()" src/App.jsx | cut -d: -f1)

if [ -z "$CONNECTOR_START" ]; then
  echo "❌ Could not find IntegrationConnectors function"
  exit 1
fi

# Find where IntegrationsPage starts (that's where IntegrationConnectors ends)
CONNECTOR_END=$(grep -n "^function IntegrationsPage()" src/App.jsx | cut -d: -f1)
CONNECTOR_END=$((CONNECTOR_END - 1))

echo "  📍 Found IntegrationConnectors at lines $CONNECTOR_START-$CONNECTOR_END"

# Create a script to replace the component
cat > /tmp/replace_component.py << 'PYEOF'
import sys

connector_start = int(sys.argv[1])
connector_end = int(sys.argv[2])

with open('src/App.jsx', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Read the new component
with open('/tmp/integration_full.js', 'r', encoding='utf-8') as f:
    new_component = f.read()

# Replace the old component
new_lines = lines[:connector_start-1] + [new_component + '\n'] + lines[connector_end:]

with open('src/App.jsx', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print("✅ Component replaced successfully")
PYEOF

python3 /tmp/replace_component.py $CONNECTOR_START $CONNECTOR_END
...
if npm run build; then
  ...
  if firebase deploy --only hosting; then
    echo ""
    echo "=============================================="
    echo "✅ COMPREHENSIVE INTEGRATION MARKETPLACE DEPLOYED!"
    echo "=============================================="
    echo ""
    echo "🎉 WHAT'S NEW:"
    echo "  📊 Usage Statistics per Integration"
    echo "  🏆 Badges (Most Used, Highest Spend, Optimal)"
    echo "  📈 License Utilization Progress Bars"
    echo "  💰 Cost Insights (Monthly, Per User)"
    echo "  💡 Optimization Suggestions"
    echo "  🔄 Last Sync Timestamps"
    echo "  🎯 Perfect Card Alignment"
    echo "  🔍 Search & Filters Working"
    echo "  📊 Enhanced Stats Header"
    echo "  🎨 Color-Coded Utilization"
    echo ""
    echo "🧪 TEST IT:"
    echo "  1. Go to /integrations"
    echo "  2. See Google Workspace & Slack with full stats"
    echo "  3. Check utilization bars"
    echo "  4. See savings opportunities"
    echo "  5. Try connecting/disconnecting integrations"
    echo ""
  fi
else
  echo "❌ Build failed"
