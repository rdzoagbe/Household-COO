...
function ExecutivePageWrapper() {
  ...
  const derived = {
    ...
    alerts: buildRiskAlerts({ ...db, tools: db.tools })
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      ...
      <nav className="relative z-50 border-b border-white/5 bg-slate-950/50 backdrop-blur-2xl sticky top-0">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div onClick={() => {}} className="flex items-center gap-4 cursor-pointer group">
              <RDLogo size="md" onClick={() => {}} />
              <div>
          ...
          <div className="grid gap-3 md:grid-cols-12">
            <div className="md:col-span-5">
              <div className="relative">
                ...
                <Input className="pl-9" placeholder={t("search_tools_owners")} value={q} onChange={(e) => setQ(e.target.value)} />
              </div>
            </div>
            <div className="md:col-span-2">
...
function EmployeesPage() {
  ...
  return (
    ...
    >
      <Card>
        <CardHeader title={t("employee_directory")} subtitle="Track departments, status, and access counts" right={<Pill tone="slate" icon={Users}>{employees.length} total</Pill>} />
        <CardBody>
          <div className="grid gap-3 md:grid-cols-12">
...
function AccessForm({ initial, tools, employees, onSubmit, onClose }) {
  ...
  return (
    ...
    >
      <div className="grid gap-3 md:grid-cols-2">
        <div>
          <div className="mb-1 text-xs font-semibold text-slate-400">{t("tool")}</div>
...
function OffboardingPage() {
  ...
  const records = access.filter((a) => a.employee_id === employeeId && a.status === "active");

  const revokeOne = (id) => muts.updateAccess.mutate({ id, patch: { status: "revoked" } });
  const revokeAll = () => {
    records.forEach((r) => revokeOne(r.id));
  ...
  return (
    ...
    >
      <Card>
        <CardHeader title="{t('revoke_access')} for departing employees" subtitle="Checklist of active access + one-click revoke" />
        <CardBody>
          {isLoading || !db ? (
            <div className="space-y-2">
              <SkeletonRow cols={6} />
            ...
            <div className="grid gap-5">
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="flex items-end justify-end gap-2">
                  <Button variant="danger" disabled={!records.length} onClick={revokeAll}>
                    <BadgeX className="h-4 w-4" />
                    Revoke All
                  </Button>
                </div>
...
function AuditExportPage() {
  ...
  const derived = useMemo(() => {
    ...
    const formerEmployeeAccess = access.filter((a) => a.derived_risk_flag === "former_employee").length;
    const spend = tools.reduce((sum, t) => sum + Number(t.cost_per_month || 0), 0);
    return { tools, access, employees: db.employees, highRiskCount, formerEmployeeAccess, spend };
  }, [db]);
  ...
  const exportTools = () => {
    ...
    downloadText(`tools_${todayISO()}.csv`, toCsv(derived.tools, cols));
  };

  const exportEmployees = () => {
    if (!derived) return;
    const cols = ["full_name", "email", "department", "role", "status", "start_date", "end_date"];
    downloadText(`employees_${todayISO()}.csv`, toCsv(derived.employees, cols));
  };
  ...
  const exportAccess = () => {
    ...
    downloadText(`access_${todayISO()}.csv`, toCsv(derived.access, cols));
  };
  ...
  return (
    ...
    >
      <Card>
        <CardHeader title="Compliance reports" subtitle="Export CSV reports (timestamped)" />
        <CardBody>
          {isLoading || !derived ? (
            <div className="space-y-2">
              <SkeletonRow cols={6} />
            ...
            <div className="grid gap-5 lg:grid-cols-12">
              <div className="lg:col-span-5">
                <div className="grid gap-3">
                  <MiniStat label="Total tools" value={derived.tools.length} />
                  <MiniStat label={t("high_risk_tools")} value={derived.highRiskCount} tone="rose" />
                  <MiniStat label={t("former_employee_access")} value={derived.formerEmployeeAccess} tone="amber" />
                  <MiniStat label={t("monthly_spend")} value={formatMoney(derived.spend)} tone="slate" />
                </div>
              </div>
              <div className="lg:col-span-7">
                <div className="grid gap-3">
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-semibold">Tools report</div>
                      <Button size="sm" variant="secondary" onClick={exportTools}>
                        <Download className="h-4 w-4" />
                        Export
                      </Button>
                    </div>
                    <div className="mt-1 text-sm text-slate-400">Inventory, ownership, status, risk, spend.</div>
                  </div>
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-semibold">Employees report</div>
                      <Button size="sm" variant="secondary" onClick={exportEmployees}>
                        <Download className="h-4 w-4" />
                        Export
                      </Button>
                    </div>
                    <div className="mt-1 text-sm text-slate-400">Directory with status and dates.</div>
                  </div>
                  <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-semibold">Access report</div>
                      <Button size="sm" variant="secondary" onClick={exportAccess}>
                        <Download className="h-4 w-4" />
                        Export
                      </Button>
                    </div>
                    <div className="mt-1 text-sm text-slate-400">Tool-to-employee access mappings and risk flags.</div>
                  </div>
                </div>
              </div>
            </div>
  ...
  const tiers = [
    {
      ...
      features: ["10 tools", "5 employees", "Basic alerts", "CSV export"],
    },
    {
      id: "pro",
      name: "Pro",
      price: "€49/mo",
      blurb: "Unlimited tracking + advanced risk",
      features: ["Unlimited tools", "Unlimited employees", "Advanced risk scoring", "Priority alerts", "Full audit reports"],
      recommended: true,
    },
    {
      id: "enterprise",
      name: "Enterprise",
      price: "€149/mo",
      blurb: "SSO/SCIM + custom features",
      features: ["Everything in Pro", "Custom integrations", "SSO & SCIM", "Dedicated support", "SLA"],
    },
  ];

  const upgrade = (id) => {
    muts.setPlan.mutate(id);
    alert("Stripe integration is mocked in this demo. Plan updated locally.");
  };

  return (
    <AppShell title="Billing" right={<Pill tone="blue" icon={CreditCard}>Plan: {plan.toUpperCase()}</Pill>}>
      <div className="grid gap-5 md:grid-cols-3">
        {tiers.map((t) => (
          <Card key={t.id} className={cx(t.recommended ? "border-blue-600/40" : "")}> 
            <CardHeader
              title={t.name}
              subtitle={t.blurb}
              right={t.recommended ? <Pill tone="blue" icon={Sparkles}>Recommended</Pill> : null}
            />
            <CardBody>
              ...
              <div className="mt-6">
                {plan === t.id ? (
                  ...
                  <Button className="w-full" variant={t.recommended ? "primary" : "secondary"} onClick={() => upgrade(t.id)}>
                    {t.id === "enterprise" ? "Contact sales" : "Upgrade"}
                  </Button>
                )}
              </div>
              <div className="mt-3 text-xs text-slate-500">Success/cancel redirects are mocked.</div>
            </CardBody>
          </Card>
        ))}
      </div>
...
const INTEGRATIONS = [
  { id: "slack", name: "Slack", desc: "Messaging + alerts", features: "Users, groups, SSO", pricing: "Free/Pro" },
  { id: "gws", name: "Google Workspace", desc: "Identity + email", features: "Users, SSO", pricing: "Paid" },
  { id: "m365", name: "Microsoft 365", desc: "Identity + apps", features: "Users, SSO", pricing: "Paid" },
  { id: "github", name: "GitHub", desc: "Source control", features: "Org members, teams", pricing: "Free/Team" },
  { id: "jira", name: "Jira", desc: "Issue tracking", features: "Users, projects", pricing: "Paid" },
  { id: "salesforce", name: "Salesforce", desc: "CRM", features: "Users, roles", pricing: "Paid" },
  { id: "hubspot", name: "HubSpot", desc: "CRM + marketing", features: "Users, roles", pricing: "Paid" },
  { id: "zoom", name: "Zoom", desc: "Meetings", features: "Users, licenses", pricing: "Paid" },
  { id: "okta", name: "Okta", desc: "SSO provider", features: "SSO, SCIM", pricing: "Paid" },
  { id: "dropbox", name: "Dropbox", desc: "Storage", features: "Users, sharing", pricing: "Paid" },
  { id: "aws", name: "AWS", desc: "Cloud", features: "Accounts, IAM", pricing: "Usage" },
  { id: "azure", name: "Azure", desc: "Cloud", features: "Accounts, IAM", pricing: "Usage" },
];
...
function IntegrationsPage() {
  ...
  return (
    <AppShell title="Integrations" right={<Pill tone="slate" icon={Plug}>Marketplace</Pill>}>
      <Card>
        <CardHeader
          title="Tool Marketplace"
          subtitle="12 SSO-enabled tools (mock connectors)"
          right={
        ...
        <CardBody>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((it) => (
              <div key={it.id} className="rounded-2xl border border-slate-800 bg-slate-950/30 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold">{it.name}</div>
                    <div className="mt-1 text-sm text-slate-400">{it.desc}</div>
                  </div>
                  <Pill tone="blue" icon={Lock}>
                    SSO
                  </Pill>
                </div>
                <div className="mt-3 text-xs text-slate-500">Syncs: {it.features}</div>
                <div className="mt-1 text-xs text-slate-500">Pricing: {it.pricing}</div>
                <div className="mt-4">
                  <Button
                    className="w-full"
                    variant="secondary"
                ...
                <div className="mt-2 text-xs text-slate-500">Requires backend functions.</div>
              </div>
            ))}
          </div>
        </CardBody>
      ...
      <Modal
        open={open}
        title={selected ? `Connect ${selected.name}` : "Connect"}
        subtitle="Enable backend functions to activate SSO sync"
        onClose={() => setOpen(false)}
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button
              variant="secondary"
              onClick={() => {
                alert("This is a frontend-only demo. Enable backend functions to connect.");
                setOpen(false);
              }}
            >
      ...
      >
        <div className="space-y-3 text-sm text-slate-300">
          <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
            ...
            <div className="mt-2 text-slate-400">1) Admin approves SSO. 2) Connector syncs users/groups. 3) We map access and score risk.</div>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
            <div className="font-semibold">What you’ll get</div>
            <div className="mt-2 flex flex-wrap gap-2">
              <Pill tone="blue" icon={Users}>User roster</Pill>
              <Pill tone="amber" icon={AlertTriangle}>Risk flags</Pill>
              <Pill tone="green" icon={BadgeCheck}>Auto offboarding</Pill>
            </div>
          </div>
        </div>
      </Modal>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      <nav className="border-b border-white/5 bg-slate-950/50 backdrop-blur-2xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
          <div onClick={() => {}} className="flex items-center gap-4 cursor-pointer">
            <RDLogo size="md" />
          ...
          <button onClick={() => {}} className="text-slate-300 hover:text-white transition-colors">← Back to Home</button>
        </div>
      </nav>
...
function LicenseManagement() {
  ...
  return (
    <AppShell currentPage="licenses">
      <div className="p-8">
        <div className="mb-8">
          <h1 className="text-4xl font-black mb-2">📜 License Management</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  ...
                  <th className="text-right py-4 px-4 text-sm font-semibold text-slate-400">{t("cost")}</th>
                ...
                {critical.map((renewal, idx) => (
                  <div key={idx} className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                      ...
                      <div>
                        <div className="text-slate-400">{t("owner")}</div>
