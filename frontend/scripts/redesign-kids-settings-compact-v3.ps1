# Household COO - Compact Kids + Settings redesign v3
# Parser-safe version. Restores failed local edits, then applies compact segmented pages.
...
# Reset failed local edits from v1/v2 while keeping helper scripts from the branch.
git restore -- 'frontend/app/(tabs)/kids.tsx' 'frontend/app/(tabs)/settings.tsx'

# -----------------------------------------------------------------------------
# Kids: compact segmented layout
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Settings: compact segmented layout
# -----------------------------------------------------------------------------
...
$settings = $settings.Replace("hidden={activeSettingsView !== ''family''}", "hidden={activeSettingsView !== 'family'}")
...
'@
$settings = $settings.Replace($oldSectionTitle, $newSectionTitle)
...
if ($settings -notmatch 'settingsSegmentWrap') { throw 'Settings tabs were not inserted.' }
