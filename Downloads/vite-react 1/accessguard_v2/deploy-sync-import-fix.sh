...
echo ""
echo -e "${BLUE}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  AccessGuard V2 — Sync + Import Fix              ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════╝${NC}"
echo ""
...
# ═══════════════════════════════════════════════════════════
# PATCH 1 — Replace ImportPage with full preview + validation
# ═══════════════════════════════════════════════════════════
patch("ImportPage: full preview + validation + import flow",
'''function ImportPage() {
    ...
    <AppShell title="Import Data" right={<Pill tone="slate" icon={Upload}>CSV</Pill>}>
      <div className="grid gap-5 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          {/* Preview table — shows before import */}
          {liveRows.length > 0 && !imported && (
            <Card>
              <CardHeader
                title="Preview"
                subtitle={`${liveRows.length} row(s) parsed from your CSV`}
                right={
          ...
          {/* Success table — shows after import */}
          {imported && (
            <Card>
              <CardHeader
                title={`✅ ${imported.count} ${imported.kind} imported successfully`}
                subtitle="Records have been added to your workspace"
        ...
        {/* RIGHT — guidelines + current counts */}
...
# ═══════════════════════════════════════════════════════════
# PATCH 2 — Replace GoogleWorkspaceSync with multi-platform
#           Directory Sync component (Google + Microsoft)
# ═══════════════════════════════════════════════════════════

# First check if old GoogleWorkspaceSync exists
GWS_MARKER = "function GoogleWorkspaceSync"
if GWS_MARKER in src:
    # Find and replace the whole component
    start = src.find("function GoogleWorkspaceSync")
    # find next function after it
    next_fn = src.find("\nfunction ", start + 10)
    old_gws = src[start:next_fn]
    patch("GoogleWorkspaceSync → DirectorySync multi-platform", old_gws,
  ...
  const PLATFORMS = [
    { id: 'google',    name: 'Google Workspace',  icon: '🔵', color: 'from-blue-600 to-blue-700',    desc: 'Sync users via Google Workspace Admin API' },
    { id: 'microsoft', name: 'Microsoft 365',      icon: '🟦', color: 'from-indigo-600 to-blue-700', desc: 'Sync users via Microsoft Entra ID (Azure AD)' },
    { id: 'okta',      name: 'Okta',               icon: '🔷', color: 'from-sky-600 to-cyan-700',    desc: 'Sync users from Okta Universal Directory' },
    { id: 'jumpcloud', name: 'JumpCloud',           icon: '🟩', color: 'from-emerald-600 to-green-700', desc: 'Sync users from JumpCloud Directory' },
  ];
  ...
  const handleSync = async () => {
    ...
    setSyncStatus(null);
    try {
      // Simulate a sync — in production this would call your backend OAuth token exchange
      // and hit the respective directory API (Google Admin SDK / MS Graph / Okta API)
      await new Promise(resolve => setTimeout(resolve, 1800));

      // Demo: generate sample users from the selected platform
      const sampleUsers = {
      ...
      const users = sampleUsers[platform] || [];
      if (users.length > 0) {
        await muts.bulkImport.mutateAsync({ kind: 'employees', records: users });
      }

      const now = new Date().toLocaleString();
      localStorage.setItem('ag_last_sync', now);
      setLastSync(now);
      setSyncStatus('success');
      setSyncMsg(`Synced ${users.length} user${users.length !== 1 ? 's' : ''} from ${selectedPlatform.name}`);
    } catch (err) {
      setSyncStatus('error');
      setSyncMsg(`Sync failed: ${err.message || 'Connection error'}`);
    } finally {
      setSyncing(false);
    }
  };
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 mb-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className={cx("w-10 h-10 rounded-xl bg-gradient-to-br flex items-center justify-center text-lg flex-shrink-0", selectedPlatform.color)}>
            {selectedPlatform.icon}
          </div>
          <div>
            <div className="font-semibold text-white text-sm">Directory Sync</div>
            <div className="text-xs text-slate-400 mt-0.5">{selectedPlatform.desc}</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <select
            value={platform}
            onChange={(e) => { setPlatform(e.target.value); setSyncStatus(null); }}
          ...
          <button
            ...
            disabled={syncing}
            className={cx(
              "inline-flex items-center gap-2 h-8 px-4 rounded-lg text-xs font-semibold transition-all",
              syncing
      ...
      {!syncStatus && lastSync && (
        <div className="mt-3 text-xs text-slate-500">Last synced: {lastSync}</div>
      )}
    </div>
  );
}
...
''')
    ...
    # GoogleWorkspaceSync doesn't exist at all — inject brand new component before DashboardPage
    NEW_COMPONENT = '''function DirectorySync() {
  const muts = useDbMutations();
  const [platform, setPlatform] = useState('google');
  const [syncing, setSyncing] = useState(false);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const users = sampleUsers[platform] || [];
      if (users.length > 0) await muts.bulkImport.mutateAsync({ kind: 'employees', records: users });
      const now = new Date().toLocaleString();
      localStorage.setItem('ag_last_sync', now);
      setLastSync(now);
      setSyncStatus('success');
...
'''
    patch("Inject DirectorySync component before DashboardPage",
        "function DashboardPage() {",
        NEW_COMPONENT + "function DashboardPage() {"
    )

# PATCH 3 — Wire DirectorySync into DashboardPage JSX (render above AIInsights)
# Find where AIInsights is rendered and add DirectorySync above it
patch("Render DirectorySync above AIInsights in DashboardPage",
    "        {/* AI-Powered Insights */}\n        <AIInsights",
    "        {/* Directory Sync */}\n        <DirectorySync />\n\n        {/* AI-Powered Insights */}\n        <AIInsights"
)
...
for name in failed:
    print(f"  \033[31m✗\033[0m  FAILED: {name}")
...
print(f"\n  \033[32mAll patches applied.\033[0m")
PYEOF
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  ✓ Both fixes deployed!                          ║${NC}"
echo -e "${GREEN}╠══════════════════════════════════════════════════╣${NC}"
echo -e "${GREEN}║  IMPORT PAGE                                     ║${NC}"
echo -e "${GREEN}║    • Import button now works (live row counter)  ║${NC}"
echo -e "${GREEN}║    • Preview table appears as you type           ║${NC}"
echo -e "${GREEN}║    • Red rows = missing required fields          ║${NC}"
echo -e "${GREEN}║    • Success table shows after import            ║${NC}"
echo -e "${GREEN}║  DIRECTORY SYNC                                  ║${NC}"
echo -e "${GREEN}║    • Dropdown: Google / Microsoft / Okta /       ║${NC}"
echo -e "${GREEN}║      JumpCloud                                   ║${NC}"
echo -e "${GREEN}║    • Sync Now imports sample users per platform  ║${NC}"
echo -e "${GREEN}║    • Last sync timestamp persisted               ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════╝${NC}"
echo ""
